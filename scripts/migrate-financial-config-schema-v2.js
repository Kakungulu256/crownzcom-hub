...
async function ensureFinancialConfigSchemaV2() {
  await safeCreate(
    'attribute longTermInterestRate',
    () => databases.createFloatAttribute(databaseId, financialConfigCollectionId, 'longTermInterestRate', false)
  );
  await safeCreate(
    'attribute longTermMaxRepaymentMonths',
    () => databases.createIntegerAttribute(databaseId, financialConfigCollectionId, 'longTermMaxRepaymentMonths', false)
  );
}
...
async function backfillFinancialConfigDocs() {
  ...
  for (const doc of docs) {
    const updates = {};
    if (doc.longTermInterestRate === undefined || doc.longTermInterestRate === null) {
      updates.longTermInterestRate = DEFAULT_FINANCIAL_CONFIG.longTermInterestRate;
    }
    if (doc.longTermMaxRepaymentMonths === undefined || doc.longTermMaxRepaymentMonths === null) {
      updates.longTermMaxRepaymentMonths = DEFAULT_FINANCIAL_CONFIG.longTermMaxRepaymentMonths;
    }
...
async function main() {
  ...
  await ensureFinancialConfigSchemaV2();
  await backfillFinancialConfigDocs();
  console.log('Financial config schema migration v2 complete.');
}
