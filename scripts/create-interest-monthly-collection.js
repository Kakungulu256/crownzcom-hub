...
async function ensureInterestMonthlyCollection() {
  const collectionId = INTEREST_MONTHLY_COLLECTION_ID || sdk.ID.unique();
  const name = 'interest_monthly';
  const permissions = [
  ...
  if (found) {
    console.log(`Interest monthly collection already exists: ${found.$id}`);
    return found.$id;
  }
...
async function main() {
  const collectionId = await ensureInterestMonthlyCollection();
  await createAttributes(collectionId);
  await createIndexes(collectionId);
  console.log(`Interest monthly collection ready: ${collectionId}`);
}
